...
/* =========================
   🔹 FILTER SEARCH
========================= */
const filteredTunjangan = computed(() => {
if (!search.value) return tunjanganList.value

  const keyword = search.value.toLowerCase()

  // 1. Exact match di title
  const exactMatch = tunjanganList.value.filter(item =>
  ...
  return tunjanganList.value.filter(item =>
  ...
  <div class="container bantuan-container">
    ...
    <div class="row">
      ...
      <div class="col-md-4 mb-4">
        <div
          v-for="item in filteredTunjangan"
          :key="item.id"
          :class="[
        ...
        >
          <div class="card-body">
            ...
            <div class="d-flex justify-content-between align-items-center mt-3">
              ...
              <div v-if="selectedItem?.id === item.id" class="selected-indicator-icon">
                ✔
              </div>
            </div>
          </div>
        ...
        <!-- 🔹 EMPTY STATE -->
        <div v-if="filteredTunjangan.length === 0" class="text-center py-5 bg-white rounded shadow-sm border">
          <p class="text-muted m-0">Data tidak ditemukan.</p>
        </div>
      </div>
      ...
      <div class="col-md-5 mb-4">
        <div class="card p-4 shadow-sm border-0 sticky-column" style="min-height: 400px;">
          ...
          <div v-else class="h-100 d-flex flex-column align-items-center justify-content-center text-muted text-center p-4">
            <h6>Pilih Tunjangan</h6>
            <p class="small">Pilih salah satu item untuk melihat detail.</p>
          </div>
