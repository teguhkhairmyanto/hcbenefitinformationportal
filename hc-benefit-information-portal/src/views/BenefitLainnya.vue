...
const filteredBenefit = computed(() => {
  if (!search.value) return benefitList.value

  const keyword = search.value.toLowerCase()

  // 1. Exact match di title
  const exactMatch = benefitList.value.filter(item =>
  ...
  return benefitList.value.filter(item =>
...
<template>
  <div class="container bantuan-container">
    ...
    <div class="mb-4">
      <SearchBar 
        v-model="search" 
        placeholder="Cari benefit lainnya..." 
      />
    </div>
    ...
    <div class="row">
      ...
      <div class="col-md-4 mb-4">
        ...
        <!-- 🔹 EMPTY STATE -->
        <div v-if="filteredBenefit.length === 0" class="text-center py-5 bg-white rounded shadow-sm border">
